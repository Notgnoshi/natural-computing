...
Note that this is memory inefficient. I believe that there should be a way to use an array of
shuffled indices as your population, and then implement the fitness function in such a way that it
knows how to retrieve the location of a city from its index. However, it runs well enough that I
won't change it now.
...
Note that the sum is negated, because we want to \textit{minimize} the fitness.\todo{What happens
    if it isn't negated? I couldn't even begin to guess.}
...
\todoinline{Copy-paste implementation and add commentary.}
...
\todoinline{
    ...
    Compare against simulated annealing (easy!)

    Talk about how getting the fitness function right really changes things. Add plots, and
    reference the plots in the fitness function section of the methodology.
}
